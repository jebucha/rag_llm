...
import torch  # for MPS support on Apple Silicon
...
# LLM model selection (sidebar) used for tokenization and querying
MODEL_CHOICES = ["mistral", "cogito", "gemma3"]
model_choice = st.sidebar.selectbox("Choose LLM model", MODEL_CHOICES)
...
def load_embedding_model(model_name: str = "all-MiniLM-L6-v2") -> SentenceTransformer:
    """
    Load and move the embedding model to MPS if available (for Apple Silicon).
    """
    model = SentenceTransformer(model_name)
    # Use MPS on Apple M1/M2 if available
...
def load_tokenizer(model_choice: str) -> AutoTokenizer:
    """
    Map logical names to Hugging Face model IDs and load matching tokenizer.
    """
    tokenizer_map = {
...
def chunk_text_by_tokens(
    ...
    """
    Split text into <= max_tokens chunks with overlap using the selected tokenizer.
    """
...
def split_json_and_plain(text: str) -> (List[str], str):
    """
    Separate JSON lines from plain text for more precise chunking.
    """
    json_lines, plain_lines = [], []
...
def super_chunk(text: str, tokenizer: AutoTokenizer) -> List[str]:
    """
    Combine JSON-aware splitting with token-based chunking.
    """
...
with tab_ingest:
    ...
    if uploaded:
        ...
        for file in uploaded:
            ...
            # Use the same tokenizer matching selected LLM for chunking
            chunks     = super_chunk(text, tokenizer)
